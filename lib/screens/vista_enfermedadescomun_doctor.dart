...
class VistaEnfermedadComun extends StatefulWidget {
  ...
  const VistaEnfermedadComun({super.key, required this.id_paciente,required this.nombre, required this.apellido, required this.idusuariodoc});

  @override
  State<VistaEnfermedadComun> createState() => _VistaEnfermedadComun();
}

class _VistaEnfermedadComun extends State<VistaEnfermedadComun> {
  ...
  String? tipoSeleccionado= 'respiratoria';
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              ...
              title: Text("Añadir Enfermedad Comun"),
                      ...
                      Padding(
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: InputDecoration(
                            labelText: "Tipo de Enfermedad comun",
                            border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(8.0), // Bordes redondeados
                            ),
                          ),
                          items: ['respiratoria', 'viral', 'bacteriana', 'digestiva','dermatológica','otros']
                      ...
                      FutureBuilder<List<EnfermedadPersistente>>(
                        ...
                        builder: (context, snapshot) {
                          if (snapshot.connectionState == ConnectionState.waiting) {
                            ...
                            return Text('No hay Enfermedad Comun disponibles');
                          } else {
                  ...
                  onPressed: () async {
                    if (selectedEnfermedadesPersistenteId != null ) {
                      ...
                      final Map<String, dynamic> data = {
                        'paciente': idPaciente,
                        'enfermedad_id': selectedEnfermedadesPersistenteId,
  ...
  Future<void> _fetchEnfermedadesPersistente({String? tipo}) async {
    ...
    try {
      final url = (tipo == null || tipo.isEmpty)
          ? '$baseUrl/usuarios/api/paciente/$idPaciente/enfermedades-comunes/?activas=true'
          : '$baseUrl/usuarios/api/paciente/$idPaciente/enfermedades-comunes/?activas=true&tipo=$tipo';
  ...
  Future<void> marcarRecuperado(int idPacienteEnfermedad) async {
    final url = Uri.parse('$baseUrl/usuarios/api/paciente-enfermedad-comun/$idPacienteEnfermedad/');

    // Obtener fecha actual en formato yyyy-MM-dd
    String fechaRecuperacion = DateFormat('yyyy-MM-dd').format(DateTime.now());
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Enfermedad marcada como recuperada")),
        );
        await _fetchEnfermedadesPersistente(); // Actualiza la lista para reflejar el cambio
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> editarEnfermedadesPersistente({required int id, required String observacion,}) async {
    ...
    final url = Uri.parse('$baseUrl/usuarios/api/paciente-enfermedad-comun/$id/');
  ...
  String _formatearFecha(String? fecha) {
    ...
    return '${fechaParseada.day.toString().padLeft(2, '0')}/'
        '${fechaParseada.month.toString().padLeft(2, '0')}/'
        '${fechaParseada.year}';
  }
                    ...
                    children: [
                          ...
                          Container(
                            ...
                            child: PopupMenuButton<String>(
                              ...
                              itemBuilder: (context) => [
                                ...
                                const PopupMenuItem(value: 'viral', child: Text('Viral')),
                                const PopupMenuItem(value: 'bacterial', child: Text('Bacteriana')),
                                const PopupMenuItem(value: 'digestiva', child: Text('Digestiva')),
                                const PopupMenuItem(value: 'dermatológica', child: Text('Dermatológica')),
                                const PopupMenuItem(value: 'otros', child: Text('Otros')),
                      ...
                      Expanded(child: isLoading
                        ...
                        itemBuilder: (context, index) {
                          String tipo = EnfermedadesPersistente[index]['tipo_enfermedad']?.toString() ?? '';
                                          ...
                                          children: [
                                            ...
                                            Row(
                                              children: [
                                                ...
                                                const SizedBox(width: 4),
                                                Text(
                                                  'Detectado el: ${_formatearFecha(EnfermedadesPersistente[index]['fecha_diagnostico'])}',
                                                  style: const TextStyle(color: Colors.black87, fontSize: 12),
                                                ),
                                              ],
                                            ),
                                    ...
                                    onPressed: () {
                                      showDialog(
                                        ...
                                        builder: (context) => AlertDialog(
                                          ...
                                          content: const Text('¿Estás seguro de que deseas marcar esta enfermedad como recuperada?'),
                                          actions: [
                                            TextButton(
                                              onPressed: () => Navigator.of(context).pop(),
                                              child: const Text('Cancelar'),
